...
const form = () => {
    ...
    useEffect(() => {
        if (query.id) {
            axios.get('/api/card/' + query.id).then(resultado => {
                const card = resultado.data

                for (let atributo in card) {
                    setValue(atributo, card[atributo])
                }
            })
        }
    }, [query.id])
    ...
    function salvar(dados) {
        axios.put('/api/card/' + query.id, dados)
        push('/card')
    }
